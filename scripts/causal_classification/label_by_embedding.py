...
def main():
    ...
    if "svm_label" not in fieldnames:
        fieldnames = [*fieldnames, "svm_label"]

    classifier = EmbeddingCausalClassifier.load()
    predictions = classifier.predict([row["sentence"] for row in rows])

    for row, prediction in zip(rows, predictions):
        row["svm_label"] = prediction
    ...
    print(f"Total {len(rows)} sentences, {total_causal} causal (svm_label) -> {CAUSAL_SENTENCES_PATH}")
