...
def extract_relations(text):
    ...
    relations = relation_extractor.extract(sens)
    return relations
...
for i in range(1, 4):
    ...
    extracted_relations.extend(extract_relations(filter_text))
...
df.to_csv(f"{ROOT_DIR}/output/relations.csv", index=False, encoding="utf-8-sig", header=True)
